    ...
    return (
      <div className="relative group">
        <input
          type={type}
          className={cn(
            "flex w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-4 text-sm text-white placeholder:text-transparent",
            "transition-all duration-300 ease-out focus:border-white/30 focus:bg-white/10 focus:outline-none focus:ring-4 focus:ring-white/5",
            hideTitle ? "py-4" : "peer pt-6 pb-2", // pt-6 and pb-2 leaves room for floating label inside
            className
          )}
          ref={ref}
          placeholder=" " // Keep this a space for peer-placeholder-shown to work
          {...props}
        />
        {!hideTitle && (
